...
void gfx_init(void)
{
    ...
    printf("phew, done waiting\n");
	hw_tail = read8(0x1000c018);
	gfx_head = gfx_queue;
    printf("lets clear some memory\n");
	memset16(gfx_queue, 0, 131072);
	memset16(gfx_regs, 0, REG_COUNT);
	// clear all registers
    printf("and those pesky registers\n");
	gfx_queue[0] = (REG_COUNT-1) << 8;
	gfx_sendfifobuf(gfx_queue, 1 + 0x1c);
    printf("all set!\n");

    printf("init the lcd device\n");
    device_ioctl(lcddev, LCDIOC_INIT, 6);
    device_ioctl(lcddev, LCDIOC_SET_BACKLIGHT, 1);
    device_ioctl(lcddev, LCDIOC_DRAW, NULL);

    printf("allocating a framebuffer\n");
    fb = malloc( 320*240 * 2 );
    printf("passing the pointer to the lcd device\n");
    device_ioctl(lcddev, LCDIOC_SET_FB, fb);

    printf("don't forget to tell the gfx chip where the fb is in memory\n");
    push1( GR_FB_LO,  ((unsigned int)fb) & 0xffff);
	push1( GR_FB_HI,  ((unsigned int)fb) >> 16 );
    push1( GR_FB_STRIDE, 480 );
    push1( GR_FB_HEIGHT, 320 );
