...
#  endif
	...
	if (tcp_flag==2) {
        ...
        fprintf(stdout,"hell no, we don't do TCP\n");
...
tryz(void)
{
	...
	for (n=zmodem_requested?15:5; 
		...
		if (tcp_socket==-1 && *tcp_buf) {
			/* we need to switch to tcp mode */
          printf("As you probably guessed, no TCP support here\n");
...
sys2(const char *s)
{
	if (*s == '!')
		++s;
    printf("system() calls are not supported on this platform!\n");
	return -1; /*  system(s); */
}
